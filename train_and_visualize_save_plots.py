...
# Train, evaluate, and save visualizations
model_names = []
accuracies = []
conf_matrices = []
...
plt.grid(axis='y')
plt.tight_layout()
plt.savefig("output_plots/accuracy_comparison.png")
plt.close()
...
for name, cm in conf_matrices:
    ...
    plt.ylabel("Actual")
    plt.tight_layout()
    plt.savefig(f"output_plots/confusion_matrix_{name}.png")
    plt.close()

print("\nAll models trained and plots saved to 'output_plots/' folder.")
